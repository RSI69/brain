...
# ---------------------------------------------------------------------------
SYSTEM_PROMPT = (
    "SYSTEM: Be direct, objective, concise. No flattery or apologies. Profanity is allowed."
)
JUDGE_RUBRIC = (
...
GEN_MODELS: Dict[str, Llama] = {}
for i in range(1, 8):
    p = os.getenv(f"GEN_MODEL_{i}")
    if p:
        GEN_MODELS[f"G{i}"] = load_llama(p, GPU_LAYERS_GEN)
if not GEN_MODELS:
    raise RuntimeError("At least one GEN_MODEL_* env var is required.")

JUDGE = load_llama(os.getenv("JUDGE_MODEL"), GPU_LAYERS_JUDGE)
...
async def chat(req: ChatRequest, request: Request):
    answer = await answer_user(req.prompt, req.max_tokens or MAX_TOKENS_GEN)
...
async def stream(req: ChatRequest, request: Request):
    async def event_source():
        answer = await answer_user(req.prompt, req.max_tokens or MAX_TOKENS_GEN)
        for chunk in re.findall(r".{1,80}", answer):
            yield f"data: {chunk}\n\n"
